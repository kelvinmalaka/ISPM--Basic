...
class CategoryCaseFile extends FilePermission {
  /**
  ...
  public function __construct(ContestCategory $category) {
    ...
    parent::__construct($category->case_file_path);
  }
  ...
  public function canBeAccessedBy(User $user): bool {
    ...
    if ($user->role === "contestant") {
      ...
      $verifiedTeams = $teams->filter(fn ($team) => $team->isRegistrationVerified());
      $contestant = $verifiedTeams->map(fn ($team) => $team->contestant->user)->first(fn ($contestant) => $contestant->is($user));
