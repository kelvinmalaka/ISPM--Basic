...
<div class="relative" x-data="{ open: false }">
    ...
    <div class="bg-white shadow-lg rounded absolute top-8 right-0 border text-left text-sm z-10 {{ isset($dropDownWidth) ? "w-$dropDownWidth" : '' }} {{ $sizes[$size] }}"
        :id="$id('dropdown')" x-show.transition="open" @click.away="open = false" x-cloak>
        {{ $slot }}
    </div>
</div>
