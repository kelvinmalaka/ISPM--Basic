                        ...
                        @if ($header->isSortable())
                            <div class="flex">
                                ...
                                <a href="#!" wire:click.prevent="sort('{{ $header->sortBy }}')" class="flex">
                                    <i
                                        class="bi-chevron-up {{ $sortBy === $header->sortBy && $sortOrder === 'asc' ? 'text-gray-900' : 'text-gray-400' }} h-4 w-4"></i>
                                    <i
                                        class="bi-chevron-down {{ $sortBy === $header->sortBy && $sortOrder === 'desc' ? 'text-gray-900' : 'text-gray-400' }} h-4 w-4"></i>
                                </a>
                            </div>
                        @else
                            {{ $header->title }}
    ...
    <tbody>
        ...
        @if (count($results))
            @foreach ($results as $result)
                <tr class="border-b border-gray-200 bg-gray-100 text-xs">
                    <td class="px-3 py-2 whitespace-no-wrap" colspan="100%">
                        {{ $result }}
                    </td>
                </tr>
            @endforeach
